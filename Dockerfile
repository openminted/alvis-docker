...
RUN wget https://github.com/jbjorne/TEES/tarball/master | tar -xvf /alvisnlp/psoft  && \
    # tar xvf master && \
    # rm -rf master && \
    mv *-TEES-*  tees && \
    cp tees.expect tees/ && \
    ...
    #xmlstarlet ed --inplace -u "/default-param-values/module/teesHome" -v /alvisnlp/psoft/tees /alvisnlp/share/default-param-values.xml && \
    #SPECIES
    wget http://download.jensenlab.org/species_tagger.tar.gz | tar -xvf /alvisnlp/psoft && \
    ...
    #install geniatagger
    wget http://www.nactem.ac.uk/tsujii/GENIA/tagger/geniatagger-3.0.2.tar.gz |  tar -xJC /alvisnlp/psoft /alvisnlp/psoft && \
    ...
    # treeTagger
    wget http://www.cis.uni-muenchen.de/%7Eschmid/tools/TreeTagger/data/tree-tagger-linux-3.2.1.tar.gz | tar -xJC /alvisnlp/psoft /alvisnlp/psoft && \
    #tar xvf tree-tagger-linux-3.2.1.tar.gz && \
    #rm tree-tagger-linux-3.2.1.tar.gz && \
    cd ../ && \
    # Yatea
    wget http://search.cpan.org/CPAN/authors/id/T/TH/THHAMON/Lingua-YaTeA-0.622.tar.gz | tar -xvf /alvisnlp/psoft && \
